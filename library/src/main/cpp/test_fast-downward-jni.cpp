...
#define LOG if (!is_profiling) std::cout
    ...
    for (unsigned i = 0; i < repeats; ++i) {
      ...
      auto j_sas = Java_com_softbankrobotics_fastdownward_FastDownwardKt_translatePDDLToSAS(
      ...
      auto j_plan = Java_com_softbankrobotics_fastdownward_FastDownwardKt_searchPlanFromSAS(
              env, nullptr, j_sas, env->NewStringUTF("astar(blind())"));
